...
export const getRecentActivity = query({
  ...
  handler: async (ctx, { organizationId, limit = 10 }) => {
    ...
    try {
      ...
      const result = auditLogs.map((log, index) => {
        let name = 'System';
        if (log.performedBy.type === 'user') {
          const user = userMap.get(log.performedBy.userId);
          if (user) {
            name =
              `${user.firstName || ''} ${user.lastName || ''}`.trim() || log.performedBy.userEmail;
          } else {
            name = log.performedBy.userEmail;
            log('warn', 'getRecentActivity', `User not found for audit log`, {
              logId: log._id,
              userId: log.performedBy.userId,
            });
          }
        } else if (log.performedBy.type === 'system') {
          name = log.performedBy.service;
        } else if (log.performedBy.type === 'ai') {
          name = `AI (${log.performedBy.model})`;
        }
