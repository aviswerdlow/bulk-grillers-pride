...
export const importLegacyCategories = mutation({
  ...
  handler: async (ctx, { organizationId, projectId, categoriesData }) => {
    ...
    // Set consistent timestamp for the entire import operation
    const now = Date.now();
    ...
    if (existingLevels.length === 0) {
      // Create default food category levels inline to avoid circular dependency
      const defaultLevels = [
        {
          level: 0,
          friendlyName: 'Aisle',
      ...
      for (const defaultLevel of defaultLevels) {
        await ctx.db.insert('categoryLevelDefinitions', {
          organizationId,
          projectId,
          level: defaultLevel.level,
          friendlyName: defaultLevel.friendlyName,
          description: defaultLevel.description,
          sortOrder: defaultLevel.level,
          icon: defaultLevel.icon,
          color: defaultLevel.color,
          isRequired: defaultLevel.isRequired,
          maxCategories: defaultLevel.maxCategories,
          isActive: true,
          metadata: {},
          version: 1,
          createdBy: user._id,
    ...
    // Parse and validate categories data
    let categories;
    try {
      categories = JSON.parse(categoriesData);
      if (!Array.isArray(categories)) {
        throw new Error('Categories data must be a JSON array');
      }
    } catch (error) {
      throw new Error(
...
function findParentCategory(category: any, categoryLevels: any, categoryMap: Map<string, any>) {
  ...
  for (const potentialParent of previousLevel) {
    ...
    if (currentLevel === 2) {
      ...
      if (
        parentName.includes('prepared') &&
        (currentName.includes('prepared') || currentName.includes('ready-to-eat'))
      ) {
        return potentialParent;
      }
      if (
        parentName.includes('bakery') &&
        (currentName.includes('bakery') || currentName.includes('grocery'))
      ) {
        return potentialParent;
      }
    }
    ...
    if (currentLevel === 4) {
      ...
      if (
        parentName.includes('boneless') &&
        (currentName.includes('steak') || currentName.includes('chicken'))
      ) {
        return potentialParent;
      }
  ...
  for (const potentialParent of previousLevel) {
    ...
    const commonWords = parentWords.filter(
      (word: string) => currentWords.includes(word) && word.length > 3
    );
